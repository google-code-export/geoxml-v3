...
*  Expanded With: Tiled KML support (Dynamic KML Layer) from MicroImages	  *                                           
******************************************************************************/
// Constructor
...
function DynamicKMLLayer(myvar, map, url, tilezoomlevel, opts){
		...
		this.foldernotset = true;
		this.opts.suppressfolders = true;
		this.baseUrl = url;
		this.opts.uniqueByName = true;
		this.geoxmls = [];
		this.buffer = 0;
		this.hider = null;
		this.filteredNames = [];
...
DynamicKMLLayer.prototype.tileSize = new google.maps.Size(256, 256);
...
DynamicKMLLayer.prototype.onRemove = function() {
	google.maps.event.removeListener(this.updater);
	};
	
DynamicKMLLayer.prototype.toggleContents = function(i,show){
	var z = this.tiledZoom;
	
	//for(var z = this.minTileLevel;z<= this.maxTileLevel;z++){
		if(this.geoxmls[z]){
			for(var y in this.geoxmls[z]){
				var col = this.geoxmls[z][y];
				for(var x in this.geoxmls[z][y]){
					if(col  && col[x]) {
						var geoxml = this.geoxmls[z][y][x];
						if(geoxml){
							geoxml.filteredNames = this.filteredNames;
							geoxml.contentToggle(i,show);
							geoxml.overlayman.currentZoomLevel = -1;
							OverlayManager.Display(geoxml.overlayman);
							}
						}
					}
				}
...
DynamicKMLLayer.prototype.toggleFolder = function(i){
	var z = this.tiledZoom;
		if(this.geoxmls[z]){
			var toggled = false;
			for(var y in this.geoxmls[z]){
...
DynamicKMLLayer.prototype.toggleAllNames = function(idx,show){
 
	var z = this.tiledZoom;
	var folder = document.getElementById(idx);
	var myvar = this.myvar;
	if(folder){
		
		for(var n=0;n<folder.names.length; n++){
			cb = Lance$(myvar+''+'CB'+escape(folder.names[n]));
			if (cb) {
				cb.checked = show;
				}
			var name = folder.names[n];
			this.showHideName(escape(name),show);
			}
		}
	else {return; }
	};	
...
DynamicKMLLayer.prototype.showHideName = function(name,show){
	var lastempty = this.filteredNames.length;
	var found = false;
	var newfiltered = [];
	for(var n=0;n<this.filteredNames.length;n++){
		if (this.filteredNames[n] == name && show){
				}
		else {
			newfiltered.push(this.filteredNames[n]);
			}
		if (this.filteredNames[n] == name){
			found = true;
			}		
		}
	if(!show && !found){ /*hide new ones from this layer*/
		newfiltered.push(name);
		}	
	this.filteredNames = newfiltered;
	var z = this.tiledZoom;
...
DynamicKMLLayer.prototype.onOverName = function(name,show,p){
	var z = this.tiledZoom;
	//for(var z = this.minTileLevel;z<= this.maxTileLevel;z++){
		if(this.geoxmls[z]){
			for(var y in this.geoxmls[z]){
...
DynamicKMLLayer.prototype.toggleNamedHilite = function(name){
	var z = this.tiledZoom;
	//for(var z = this.minTileLevel;z<= this.maxTileLevel;z++){
		if(this.geoxmls[z]){
			for(var y in this.geoxmls[z]){
...
DynamicKMLLayer.prototype.onOutName = function(name,show,p){
	var z = this.tiledZoom;
	if(typeof this.tiledZoom == "undefined"){
		z = this.getZoom();
		}
	//for(var z = this.minTileLevel;z<= this.maxTileLevel;z++){
		if(this.geoxmls[z]){
...
DynamicKMLLayer.prototype.getZoom = function() {
	if(this.map){}
	else { this.map = this.myMap; }
	...
	for(var y = startcol;y <= endcol;y++){
		...
		for(var x = startrow;x <= (endrow);x++){
			...
			if(col  && col[x]) {
				...
				this.geoxmls[z][y][x].filteredNames = this.filteredNames;
				this.geoxmls[z][y][x].forcefoldersopen = this.forcefoldersopen;
				this.geoxmls[z][y][x].forcefoldersclosed = this.forcefoldersclosed;
				if(this.forcefoldersopen){
					this.geoxmls[z][y][x].forceFolders(true);
					//console.log("force open");
					}
				if(this.forcefoldersclosed){
					this.geoxmls[z][y][x].forceFolders(false);
				...
				if(typeof y!= "undefined"){
					...
					opts.tileset = this.myvar;
					//opts.allfoldersopen = trcomue;
					opts.allfoldersopen = this.forcefoldersopen;
					opts.allfoldersclosed = this.forcefoldersclosed;
					if(this.filteredNames.length){
						opts.filteredNames = this.filteredNames;
						}
					var base;
					if (typeof this.baseUrl == "string") {
						...
						var size = this.baseUrl.length;
						var mode = (x + y) / size;
						var num = Math.floor(size * (mode - parseInt(mode)));
						base = this.baseUrl[num];
						}
					this.geoxmls[z][y][x] = new GeoXml(name ,this.map, base+"/"+z+"/"+x+"/"+y+".kml",opts );
					this.geoxmls[z][y][x].allRemoved = false;
					this.geoxmls[z][y][x].tilesetObj = this;
...
DynamicKMLLayer.prototype.hide = function(){
	...
	for(var f=0;f<this.folderCBNames.length;f++) {
		var obj = Lance$(this.folderCBNames[f]);
		if(obj){
			obj.checked = false;
			}
		}
	for(var n = 0;n < this.allNames.length; n++){
		var mid = this.myvar+'CB'+this.allNames[n];
		var obj = Lance$(mid);
		if(obj){
			obj.checked = false;
			}
	...
	if(this.allNames.length > 0){
		for(var n =0;n< this.allNames.length;n++){
			this.showHideName(this.allNames[n],false);
			}
		}
	else {
		var z = this.tiledZoom;
...
DynamicKMLLayer.prototype.show = function(){
	...
	if(this.allNames.length > 0){
		for(var n =0;n< this.allNames.length;n++){
			this.showHideName(this.allNames[n],true);
			}
		}
	else {
		this.makeVisible();
...
DynamicKMLLayer.prototype.makeVisible = function(){
	var z = this.tiledZoom;
	if(typeof this.tiledZoom == "undefined"){
		z = this.getZoom();
		}
...
function GeoXml(myvar, map, url, opts) {
	...
	this.rectangleLegend = false;
	if (typeof opts.rectangleLegend) {
		this.rectangleLegend = this.opts.rectangleLegend;
		}
	...
	this.onlyLegend = false;
	if (typeof opts.onlylegend != "undefined" && opts.onlylegend == true){
		this.onlyLegend = true;
		}
		
	this.showUnique = true;
	if (typeof opts.uniqueByName != "undefined" && opts.uniqueByName == false){
		this.showUnique = false;
		}
	if (this.tileset && typeof this.tileset!="undefined"){
		this.tilesetObj = eval(this.tileset);
		}
	...
	// sidebar
	this.sidebarfn = this.addSidebar;
	this.filteredNames = [];
	this.folderCBNames = [];
	if (this.opts.filteredNames){
		this.filteredNames = this.opts.filteredNames;
		}
	// elabel options 
	...
	this.suppressfolders = false;
	if (opts.suppressfolders){
		this.suppressfolders = true;
		}
    this.forcezoom = false;
	if(this.opts.forcezoom){
		this.forcezoom = true;
		}
   this.zIndex = 0;
   if(typeof this.opts.zIndex!= "undefined"){
		this.zIndex = parseInt(this.opts.zIndex);
		}
	else {
		var chunk = myvar.split("[");
		var mid = chunk[1];
		if(mid) {
			var strnum = mid.split("]");
			this.zIndex = parseInt(strnum[0]) * 10000;
			}
		}
	  if(typeof this.opts.dohilite != "undefined" && this.opts.dohilite==true){
		this.dohilite = true;
		}
	  this.onclickfollowlinks = true;
	  if(typeof this.opts.onclickfollowlinks != "undefined"){
		this.onclickfollowlinks = this.opts.onclickfollowlinks;
		this.opts.markerfollowlinks = this.onclickfollowlinks;
		}
	  ...
	  if (this.opts.singleInfoWindow) {
		this.singleInfoWindow = this.opts.singleInfoWindow;
		}
	...
	this.publishdirectory = "http://www.microimages.com/webmap/TNT78/GoogleMaps/";
	topwin = top;
	...
	if(this.tilesetObj){
		this.tilesetObj.foldericon = this.foldericon;
		this.tilesetObj.folderclosedicon = this.folderclosedicon;
		}
	...
	this.iwwidth = this.opts.iwwidth;
...
GeoXml.prototype.createMarker = function(point, name, desc, styleid, idx, instyle, visible, kml_id, markerurl,snip) {
		...
		if (instyle && typeof instyle.url == "undefined"){
			...
			m.title = name;
			m.id = kml_id;
			var obj = { "type": "label", "title": name, "description": name, "href": href, "shadow": shadow, "visibility": visible, "x": point.x, "y": point.y, "id": m.id };
			this.kml[idx].marks.push(obj);
			var parm;
			 
			//var blob = '<span style="border-left:' + this.sidebariconheight + "px solid " + style.color + ';">&nbsp;</span>';
			...
			var desc2="";
			if(this.tileset){
				parm = this.tileset + "$$$" + name + "$$$label$$$" + n + "$$$" + blob + "$$$" + visible + "$$$null$$$" + desc2;
				m.sidebarid = this.tileset + "sb" + n;
				}
			else {
				parm = this.myvar + "$$$" + name + "$$$label$$$" + n + "$$$" + blob + "$$$" + visible + "$$$null$$$" + desc2;
		...
		//	if(console) { console.log(point + " "+name); }
			return;
			}
	    ...
	    if (this.opts.baseicon) {
			var scaledSize = new google.maps.Size(32*scale, 32*scale);
			var anchor = new google.maps.Point(16*scale, 16*scale);
			var origin = new google.maps.Point(0, 0);
			if (scale){
				if(instyle && instyle.scaledSize){
					scaledSize = instyle.scaledSize;
					}
				}
			else {
			...
			if(this.opts.baseicon.anchor){
				anchor = this.opts.baseicon.anchor;
				}
				var bicon = new google.maps.MarkerImage("http://maps.google.com/mapfiles/kml/pal3/icon40.png",
				new google.maps.Size(this.opts.baseicon.size.height,this.opts.baseicon.size.width), origin, anchor, scaledSize);
			scale = 1;
			}
		icon = bicon;	
		...
		if (awidth > 0){
			html = "<div style = 'max-width:" + awidth + "px'>" + "<h1 " + this.titlestyle + ">" + name + "</h1>";
			}
		else {
			html = "<div>" + "<h1 " + this.titlestyle + ">" + name + "</h1>";
			}
		if(name != desc){
		...
		if(this.opts.markerfollowlinks && markerurl && typeof markerurl=="string" && markerurl !=""){
			if(markerurl!=''){
				m.url = markerurl;
    ...
    // Infowindow-opening event handler
			google.maps.event.addListener(m, 'click', function() {
				m.infoWindow.setPosition(point);
				m.infoWindow.open(this.map, this);
				if (!!geoxml.singleInfoWindow) {
					var ts = geoxml.tilesetObj;
					if(ts && typeof ts != "undefined"){
						if (!!ts.lastMarker && !!ts.lastMarker.infoWindow) {
							ts.lastMarker.infoWindow.close();
							}
						ts.lastMarker = m;
						}
					else {
	    ...
	    if (this.opts.sidebarid) {
			var folderid;
			if(this.tileset){
				folderid = this.tileset + "_folder" + idx;
				}
			else {
			...
			var blob;
			if(this.truelegendsize && this.opts.baseicon){
				blob = "&nbsp;<img style=\"vertical-align:text-middle;padding:0;margin:0\" height=\""+this.opts.baseicon.size.height+"\" border=\"0\" src=\"" + href + "\">&nbsp;";
				}
			else {
				blob = "&nbsp;<img style=\"vertical-align:text-top;padding:0;margin:0\" height=\""+this.sidebariconheight+"\" border=\"0\" src=\"" + href + "\">&nbsp;";
				}
	        ...
	        m.geoxml = this;
			m.onOver = function(e,force) {
					if(this.geoxml.dohilite || force){
...
GeoXml.prototype.processLine = function (pnum, lnum, idx){
	...
	//alert(op.zindex);
	p = new google.maps.Polyline( {map:this.map,path:line,strokeColor:op.color,strokeWeight:op.width,strokeOpactiy:op.opacity,zIndex:op.zindex});
	p.bounds = op.pbounds;
	p.id = op.id;
	var nhtml = "";
	var n = this.overlayman.markers.length;
	...
	if (desc && typeof desc !="undefined"){
		if (desc != op.name){
			if (awidth){
				html += "<div style='font-family: Arial, sans-serif;font-size: small;max-width:"+awidth+"px;'>"+desc+"</div>";
				}
			else {
				html += "<div style='font-family: Arial, sans-serif;font-size: small;'>"+desc+"</div>";
				}
			}
		}
	if(lnum == 0){
		...
		if(this.sidebarsnippet){
			...
			if(this.tileset){
				parm =  this.tileset+"$$$" +op.name + "$$$polyline$$$" + n +"$$$" + blob + "$$$" + op.visibility + "$$$" + pnum + "$$$" + desc2;
				this.latestsidebar = this.tileset +"sb"+n;
				}
			else {
	...
	if(this.clickablelines){
  	...
  	p.onOver = function(e, force){
		var pline = this.geoxml.polylines[this.pnum];
		if(this.geoxml.dohilite || force){
			if(this.hidden!=true){
		...
		if(p.title){
			GeoXml.tooltip.show(p.title,e);
			}
		if(this.mess) { this.geoxml.mb.showMess(this.mess,5000); } else { 
			//this.title = "Click for more information about "+this.mytitle; 
			}
		};                                     
					  
  	p.onOut = function(e,force){ 
		if(this.geoxml.dohilite || force){
			var pline = this.geoxml.polylines[this.pnum];
			if(this.hidden!=true){
			//alert(pline.lineidx);
 ...
 //	google.maps.event.addListener(p,"mouseover",p.onOver);
	...
	if(this.tilesetObj){ google.maps.event.addListener(p,"mouseover",function(){ that.tilesetObj.onOverName(p.title); });}
	else { google.maps.event.addListener(p,"mouseover",function() {that.onOverName(p.title);}); }
	
	if(this.tilesetObj){ google.maps.event.addListener(p,"mouseout",function(){ that.tilesetObj.onOutName();} );}
	else { google.maps.event.addListener(p,"mouseout",function() {that.onOutName(p.title);}); }
  	...
  	google.maps.event.addListener(p,"click", 
		function(point) {
			...
			if(this.geoxml.clickablelines||doit){ 
				if(this.geoxml.singleInfoWindow){
					if (this.geoxml.tilesetObj){
						if(!!this.geoxml.tilesetObj.lastMarker && !!this.geoxml.tilesetObj.lastMarker.infoWindow){
							this.geoxml.tilesetObj.lastMarker.infoWindow.close();
							}
						this.geoxml.tilesetObj.lastMarker = p;
						}
					else {
						if (!!this.geoxml.lastMarker && !!this.geoxml.lastMarker.infoWindow){
							this.geoxml.lastMarker.infoWindow.close();
							}
						this.geoxml.lastMarker = p;
						}
					}
...
GeoXml.prototype.createPolyline = function(lines,color,width,opacity,pbounds,name,desc,idx, visible, kml_id, zindex) {
    var p = {};
...
GeoXml.prototype.finishPolygonJSON = function(op,idx,updatebound,lastpoly) {
  ...
  if(desc != op.name){
	if(this.iwwidth){
		html += "<div style='font-family: Arial, sans-serif;font-size: small;max-width:"+this.iwwidth+"px;'>"+desc+"</div>";
		}
	else {
		html += "<div style='font-family: Arial, sans-serif;font-size: small;'>"+desc+"</div>";
		}
 ...
 // if(!op.fill){ p.fillOpacity = 0.0; }
  ...
  polygon.url = "";
	...
	if (that.onclickfollowlinks){
	 	if(polygon.title.substring(0,7) == "http://"){
		 	polygon.url = polygon.title;
			polygon.infoWindow = null;
		 	}
		
		if (polygon.title == that.unnamedarea){
			polygon.title = "";
			polygon.infoWindow = null;
			}
		}
				
	polygon.onOver = function(e,force){ 
		if(this.geoxml.dohilite || force){
			if(this.sidebarid){
				var bar = Lance$(this.sidebarid);
				if(!!bar){
  ...
  polygon.onOut = function(e,force){ 
	if(this.geoxml.dohilite || force){
		if(this.sidebarid){
			var bar = Lance$(this.sidebarid);
			if(!!bar){
	...
	polygon.onClick = function(point) {
		...
		if(!point){ 
			...
			if (this.geoxml.onclickfollowlinks && polygon.url != ''){
				if (polygon.url != "") {
					window.open(polygon.url,"_blank");
					}
				}
			else {
				if(this.geoxml.clickablepolys && this.infoWindow){ 
					if (!!this.geoxml.singleInfoWindow) {
						if(this.geoxml.tilesetObj){
							if (!!this.geoxml.tilesetObj.lastMarker && !!this.geoxml.tilesetObj.lastMarker.infoWindow) {
								this.geoxml.tilesetObj.lastMarker.infoWindow.close();
								}
							this.geoxml.tilesetObj.lastMarker = this;
							}
						else {
							if (!!this.geoxml.lastMarker && !!this.geoxml.lastMarker.infoWindow) {
								this.geoxml.lastMarker.infoWindow.close();
...
//	google.maps.event.addListener(polygon,"mouseover",polygon.onOver);
	...
	if(this.tilesetObj){ google.maps.event.addListener(polygon,"mouseover", function() { that.tilesetObj.onOverName(polygon.title); });}
	else { google.maps.event.addListener(polygon,"mouseover",function() {that.onOverName(polygon.title);}); }
	
	if(this.tilesetObj){ google.maps.event.addListener(polygon,"mouseout",function(){ that.tilesetObj.onOutName(polygon.title); });}
	else { google.maps.event.addListener(polygon,"mouseout",function() {that.onOutName(polygon.title);}); }
...
if(this.opts.polylabelclass && newgeom ) {
  ...
  if (this.basesidebar &&  newgeom) { 
   ...
   if(this.sidebarsnippet){
	...
	if(this.tileset){
		parm =  this.tilset+"$$$" +op.name + "$$$polygon$$$" + n +"$$$" + blob + "$$$" +op.visibility+"$$$null$$$"+desc2; 
		}
	else {
...
GeoXml.prototype.finishLineJSON = function(po, idx, lastlinename){
	...
	if(this.opts.basesidebar){
  	...
  	m.onOver = function(e,force){
		...
		if(this.mess) { 
			this.geoxml.mb.showMess(this.mess,5000); } else { 
			//this.title = "Click for more information about "+this.mytitle;
			}
		};
  	m.onOut = function(e,force){ 	
	...
	if(this.tilesetObj){ google.maps.event.addListener(m,"mouseover",function(){ that.tilesetObj.onOverName(m.title); });}
	else { google.maps.event.addListener(m,"mouseover",function() {that.onOverName(m.title);}); }
	
	if(this.tilesetObj){ google.maps.event.addListener(m,"mouseout",function(){that.tilesetObj.onOutName(m.title);});}
	else { google.maps.event.addListener(m,"mouseout",function() {that.onOutName(m.title);}); }
	...
	if(this.clickablelines){
  		google.maps.event.addListener(m,"click", function(point) {
				if (m.geoxml.singleInfoWindow){
					if(m.geoxml.tilesetObj){
						if(!!m.geoxml.tilesetObj.lastMarker && !!m.geoxml.tilesetObj.lastMarker.infoWindow) {
							m.geoxml.tilesetObj.lastMarker.infoWindow.close(); 
							}
						}
					else {
				...
				if (m.geoxml.tilesetObj){
					m.geoxml.tilesetObj.lastMarker = m;
					}
				else {
	...
	if(that.basesidebar && lineisnew) {
		...
		if(this.tileset){
			parm =  that.tileset+"$$$" +po.name + "$$$polyline$$$" + n +"$$$" + blob + "$$$" +po.visibility+"$$$"+(that.polylines.length-1)+"$$$"+desc2;
			}
		else {
...
GeoXml.prototype.setFolders = function() {
	var that = this;
	 if(that.tilesetObj){
		if(that.tilesetObj.foldernotset){
			var f = Lance$(that.tilesetObj.myvar+"_folder");
			if (f) {
				if(that.tilesetObj.forcefoldersopen){
					Lance$(that.tilesetObj.myvar+"_folder").style.display = "";
					}
				if(that.tilesetObj.forcefoldersclosed){
					Lance$(that.tilesetObj.myvar+"_folder").style.display = "none";
					}
				}
			that.tilesetObj.foldernotset=false;
			}
		}
...
GeoXml.prototype.toggleFolder = function(i){
	...
	if (sb) {}
		...
		if(tntLayerControlSizer){
			if(f.style.display != "none"){
				var tlc=document.getElementById("tntLayerControl");
				tlc.style.height="";
				}
			}
		}
	...
	if(f){	
		...
		if (sb) {}
			...
			if(tntLayerControlSizer){
				if(resizinglc){clearTimeout(resizinglc); }
				resizinglc = setTimeout(function(){ tntLayerControlSizer(); },55);
				}
			}
		return false;
	};
	
GeoXml.prototype.forceFolders = function (show) {
	var len = this.kml.length;
	for(var i=0;i<len;i++){
		this.kml[i].open = show;
		}
	};
	
GeoXml.prototype.toggleFolders = function(){
	var fid = this.myvar+"_folder";
	var f = Lance$(fid); 
	fid = this.myvar+"FB";
	var folderimg = Lance$(fid);
	if(f.style.display == "none"){
			...
			if(folderimg) {
				...
				for(var v =0;v < this.folderIds.length;v++){	
					Lance$(this.folderIds[v]).style.display="block";
					}
				}
			this.forcefoldersopen = true;
			this.forcefoldersclosed = false;
			}
	else{ 
		f.style.display = "none"; 
		if (folderimg){
			...
			for(var v =0;v < this.folderIds.length;v++){	
				Lance$(this.folderIds[v]).style.display="none";
				}	
			}
		this.forcefoldersclosed = true;
		this.forcefoldersopen = false;
		}
	 var sb = document.getElementById("tntSidebar");
	 if (sb) {
	  } else {
		if (tntLayerControlSizer) {
		  if (resizinglc) {
			clearTimeout(resizinglc);
		  }
		  resizinglc = setTimeout(function() {
			tntLayerControlSizer();
		  }, 55);
		}
	  }
	};	
	
DynamicKMLLayer.prototype.toggleFolders = function(){
	var fid = this.myvar+"_folder";
	var f = Lance$(fid); 
	//var tb = Lance$(this.myvar+"TB"+i);
	fid = this.myvar+"FB";
...
GeoXml.prototype.show = function(keepFilters){
	if(this.basesidebar){
		if(this.tilesetObj){
			}
		else {
			...
			for(var cb=0;cb<this.allNames.length;cb++) {
				var cbid = this.myvar+"CB"+this.allNames[cb];
				var obj = Lance$(cbid);
				if(obj){
					obj.checked = true;
					for(var v=0;v<this.filteredNames.length;v++){
						if(this.filteredNames[v] == this.allNames[cb]){
							obj.checked = false;
							break;
							}	
						}
...
GeoXml.prototype.hide = function(keepFilters){
	if(this.basesidebar){
		if(this.tilesetObj){
			}
		else {
...
GeoXml.prototype.contentToggle = function(i,show){
	...
	if(this.basesidebar && this.tilesetObj) {
		var folderid = myvar+"_folder"+i;
		var folderobj = Lance$(folderid);
		if(folderobj && folderobj.names){
			for(var n = 0; n<folderobj.names.length;n++){
				cb = Lance$(myvar+''+'CB'+escape(folderobj.names[n]));
				if(cb && typeof cb != "undefined"){
					var filtered = false;
					for (var ni=0;ni<this.filteredNames.length;ni++){
						if(escape(folderobj.names[n]) == this.filteredNames[ni]){
							filtered = true;
							break;
							}
						}
	...
	if(typeof f == "undefined"){
		this.mb.showMess("KML content cannot be shown",4000);
		return;
		}
	...
	if(show){
		...
		for (j=0;j<f.length;j++){
		   var filtered = false;
			for (var n=0;n<this.filteredNames.length;n++){
				if(this.overlayman.markers[f[j]].idx == this.filteredNames[n]){
					filtered = true;
					break;
					}
				}
...
GeoXml.prototype.toggleNamedHilite = function(name){ // if a is not defined then p will be.
	for(var m = 0;m< this.overlayman.markers.length; m++){
		var marker = this.overlayman.markers[m];
		if (marker.onOver){
			if (marker.idx == name){
				if (marker.lit) {
					marker.onOut(null,true);
					marker.lit = false;
					}
				else {
					marker.onOver(null,true);
					marker.lit = true;
					}
				}
			}
 ...
 GeoXml.prototype.showHideName = function(name,show){  
	var neverfound = true;
	var newfilters = [];
	for(var n=0;n<this.filteredNames.length;n++){
		if (this.filteredNames[n] == name){ 
		...
		else { newfilters.push(this.filteredNames[n]);}
		}
		
	if(neverfound && !show){ /*hide new ones from this layer*/
		newfilters.push(name);
		}	
	this.filteredNames = newfilters;
	
	/*	do current ones */	
	for(var m = 0;m< this.overlayman.markers.length; m++){
		var marker = this.overlayman.markers[m];
		if (marker.idx == name){
			this.showHide(m,show);
			}
		}
		 
	};

 GeoXml.prototype.onOverName = function(name,show){ // if a is not defined then p will be.
 	var	tname = unescape(name);
	for(var m = 0;m< this.overlayman.markers.length; m++){
		var marker = this.overlayman.markers[m];
		if (marker.title == tname){
			marker.onOver(show);
			}
		}
	};
	
 GeoXml.prototype.onOutName = function(name,show){ // if a is not defined then p will be.
 	var	tname = unescape(name);
	for(var m = 0;m< this.overlayman.markers.length; m++){
		var marker = this.overlayman.markers[m];
		if (marker.title == tname){
...
GeoXml.prototype.addSidebar = function(myvar, name, type, e, graphic, ckd, i, snippet, destnum) {
	var that = this;
	if(this.tileset) {
		myvar = this.tileset;
		that = this.tilesetObj;
		}
	else { myvar = this.myvar; }
	var truename = name;
 	name = name+"_"+destnum; //unique within a folder
	var check = "checked";
	var name2 = "";
	if(ckd=="false"){ check = ""; }
	if(this.showUnique || this.tileset){ 
		for(var n=0;n<this.filteredNames.length;n++){
			if (escape(name) == this.filteredNames[n]){
				check = "";
				}
			}
		var alreadyfound = false;
		for (var n=0;n<that.allNames.length;n++){
			if(escape(name) == that.allNames[n]){
				alreadyfound = true;
				}
			}
		if(!alreadyfound){
			that.allNames.push(escape(name));
			}
		name2 = GeoXml.stripHTML(name);
		}
	var h="";
	var mid;
	if(name2 != ""){
	...
	if(this.showUnique || this.tileset) {
		if (this.onlyLegend) {
			switch(type) {
				case  "marker" :  h = '<li id="'+mid+'" onmouseover="' + myvar+ '.onOverName(\''+escape(name)+'\');" >'+ graphic +'&nbsp;'+ truename + '</a>'+snippet+'</li>';
				break;
				case "label": 
				h = '<li id="'+mid+'" ><span style="margin-top:6px;"><a href="#" onclick="'+myvar+ '.toggleNamedHilite(\''+escape(name)+'\');return false;">&nbsp;<div></div>' + graphic +'&nbsp;'+ truename + '</a></span></nobr>'+snippet+'</li>';
			//onmouseout="' + myvar+ '.onOutName(\''+escape(name)+'\');" onmouseover="' myvar+ '.onOverName(\''+escape(name)+'
				break;
				case  "polyline" :  h = '<li id="'+mid+'"><span style="margin-top:6px;"><a href="#" onclick="'+myvar+ '.toggleNamedHilite(\''+escape(name)+'\');return false;">' + graphic +'&nbsp;'+ truename + '</a></span>'+snippet+'</li>';
				break;
				case "polygon": h = '<li id="'+mid+'" ><span style="margin-top:6px;"><a href="#" onclick="'+myvar+ '.toggleNamedHilite(\''+escape(name)+'\');return false;">' + graphic +'&nbsp;'+ truename + '</a></span></nobr>'+snippet+'</li>';
				break;
				case "groundoverlay": h = '<li id="'+mid+'"><span style="margin-top:6px;"><a href="#" onclick="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'zoomto\');return false;">' + graphic +'&nbsp;'+ truename + '</a></span>'+snippet+'</li>';
				break;
				case "tiledoverlay": h = '<li id="'+mid+'"><nobr><span style="margin-top:6px;"><a href="#" oncontextMenu="'+myvar+'.upgradeLayer('+i+');return false;" onclick="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'zoomto\');return false;">'+GeoXml.WMSICON +'&nbsp;'+ truename + '</a><br />'+ graphic +'</span>'+snippet+'</li>';
				break;
				}
			}
		else {
			switch(type) {
			//onmouseout="' + myvar+ '.onOutName(\''+escape(name)+'\');" 
				case  "marker" :  h = '<li id="'+mid+'" onmouseover="' + myvar+ '.onOverName(\''+escape(name)+'\');" ><input id="'+myvar+'CB'+escape(name)+'" type="checkbox" style="vertical-align:middle" '+check+' onclick="'+myvar+'.showHideName(\''+escape(name)+'\',this.checked)"><a href="#" onclick="return false;">'+ graphic +'&nbsp;'+ truename + '</a>'+snippet+'</li>';
				break;
				case "label": h = '<li id="'+mid+'" ><input id="'+myvar+'CB'+escape(name)+'" type="checkbox" '+check+' onclick="'+myvar+'.showHideName(\''+escape(name)+'\',this.checked)" /><span style="margin-top:6px;"><a href="#" onclick="'+myvar+ '.toggleNamedHilite(\''+escape(name)+'\');return false;">' + graphic +'&nbsp;'+ truename + '</a></span></nobr>'+snippet+'</li>';
			//onmouseout="' + myvar+ '.onOutName(\''+escape(name)+'\');" onmouseover="' myvar+ '.onOverName(\''+escape(name)+'
				break;
				case  "polyline" :  h = '<li id="'+mid+'"><input id="'+myvar+'CB'+escape(name)+'" type="checkbox" '+check+' onclick="'+myvar+'.showHideName(\''+escape(name)+'\',this.checked)"><span style="margin-top:6px;"><a href="#" onclick="'+myvar+ '.toggleNamedHilite(\''+escape(name)+'\');return false;">' + graphic +'&nbsp;'+ truename + '</a></span>'+snippet+'</li>';
				break;
				//onmouseout="' + myvar+ '.onOutName(\''+escape(name)+'\');" 
				case "polygon": h = '<li id="'+mid+'" ><input id="'+myvar+'CB'+escape(name)+'" type="checkbox" '+check+' onclick="'+myvar+'.showHideName(\''+escape(name)+'\',this.checked)" /><span style="margin-top:6px;"><a href="#" onclick="'+myvar+ '.toggleNamedHilite(\''+escape(name)+'\');return false;">' + graphic +'&nbsp;'+ truename + '</a></span></nobr>'+snippet+'</li>';
				break;
				case "groundoverlay": h = '<li id="'+mid+'"><input id="'+myvar+''+e+'CB" type="checkbox" '+check+' onclick="'+myvar+'.showHideName(\''+escape(name)+'\',this.checked)"><span style="margin-top:6px;"><a href="#" onclick="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'zoomto\');return false;">' + graphic +'&nbsp;'+ truename + '</a></span>'+snippet+'</li>';
				break;
				case "tiledoverlay": h = '<li id="'+mid+'"><nobr><input id="'+myvar+''+e+'CB" type="checkbox" '+check+' onclick="'+myvar+'.toggleOff('+e+',this.checked)"><span style="margin-top:6px;"><a href="#" oncontextMenu="'+myvar+'.upgradeLayer('+i+');return false;" onclick="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'zoomto\');return false;">'+GeoXml.WMSICON +'&nbsp;'+ truename + '</a><br />'+ graphic +'&nbsp;'+'</span>'+snippet+'</li>';
				break;
				}
			}
		}
	else {
		switch(type) { //not tested recently
				case  "marker" :  h = '<li id="'+mid+'" onmouseover="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'mouseover\');return false;" ><input id="'+myvar+e+'CB" type="checkbox" style="vertical-align:middle" '+check+' onclick="'+myvar+'.showHide('+e+',this.checked)"><a href="#" onclick="return false;">'+ graphic + truename + '</a>'+snippet+'</li>';
				break;
				case  "label" :  h = '<li id="'+mid+'" onmouseover="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'mouseover\');return false;" ><input id="'+myvar+e+'CB" type="checkbox" style="vertical-align:middle" '+check+' onclick="'+myvar+'.showHide('+e+',this.checked)"><a href="#" onclick="return false;">'+ graphic + truename + '</a>'+snippet+'</li>';
				break;
				case  "polyline" :  h = '<li id="'+mid+'"><input id="'+myvar+e+'CB" type="checkbox" '+check+' onclick="'+myvar+'.showHide(\''+e+'\',this.checked,'+i+')"><span style="margin-top:6px;"><a href="#" onclick="'+myvar+ '.toggleNamedHilite(\''+escape(name)+'\');return false;">&nbsp;' + graphic + truename + '</a></span>'+snippet+'</li>';
				break;
				case "polygon": h = '<li id="'+mid+'" ><input id="'+myvar+e+'CB" type="checkbox" '+check+' onclick="'+myvar+'.showHide('+e+',this.checked)" /><span style="margin-top:6px;"><a href="#" onclick="'+myvar+ '.toggleNamedHilite(\''+escape(name)+'\');return false;">&nbsp;' + graphic + truename + '</a></span></nobr>'+snippet+'</li>';
				break;
				case "groundoverlay": h = '<li id="'+mid+'"><input id="'+myvar+''+e+'CB" type="checkbox" '+check+' onclick="'+myvar+'.showHide('+e+',this.checked)"><span style="margin-top:6px;"><a href="#" onclick="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'zoomto\');return false;">&nbsp;' + graphic + truename + '</a></span>'+snippet+'</li>';
				break;
				case "tiledoverlay": h = '<li id="'+mid+'"><nobr><input id="'+myvar+''+e+'CB" type="checkbox" '+check+' onclick="'+myvar+'.toggleOff('+e+',this.checked)"><span style="margin-top:6px;"><a href="#" oncontextMenu="'+myvar+'.upgradeLayer('+i+');return false;" onclick="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'zoomto\');return false;">'+GeoXml.WMSICON +'&nbsp;'+ truename + '</a><br />'+ graphic +'</span>'+snippet+'</li>';
				break;
				}
...
GeoXml.prototype.parseXML = function( data ) {
		...
		if ( !xml || !xml.documentElement || xml.getElementsByTagName( "parsererror" ).length ) {
			...
			data = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><InvalidXML></InvalidXML>";
			if ( window.DOMParser ) { // Standard
				tmp = new DOMParser();
				xml = tmp.parseFromString( data , "text/xml" );
			} else { // IE
...
GeoXml.prototype.handleGeomark = function (mark, idx, trans) {
	...
	if(this.iwheight){
		desc = "<div id='currentwindow' style='overflow:auto;max-height:"+this.iwheight+"px' >"+dc.desc+"</div> ";
		}
	else {
		desc = "<div id='currentwindow'>"+dc.desc+"</div> ";
		}
...
GeoXml.prototype.handlePlacemarkGeometry = function(mark, geom, idx, depth, fullstyle) {
	  ...
	  var iwheightstr;
		if (this.iwheight != 0){
			iwheightstr = "style='overflow:auto;max-height:"+this.iwheight+"px'";
			}
        if (typeof desc == "undefined" || !desc || this.opts.makedescription) {
            var dc = that.makeDescription(mark, "");
			
            desc = "<div id='currentpopup' "  + iwheightstr + ">" + dc.desc + "</div> ";
            if (!name && dc.title) {
                name = dc.title;
			...
			if(this.iwheight){
				desc = "<div id='currentpopup' " + iwheightstr+" >" + desc + "</div> ";
				}
			}
        ...
        if (nn.match(/^(LineString)/i)||nn.match(/^(gml:LineString)/i)) {
            ...
            if (typeof name == "undefined") { name = that.unnamedpath; }
            if (!!that.opts.createpolyline) {
                that.opts.createpolyline(lines, color, width, opacity, pbounds, name, desc, idx, visible, kml_id, zindex);
            } else {
                that.createPolyline(lines, color, width, opacity, pbounds, name, desc, idx, visible, kml_id, zindex);
            }
        }
	//	alert(coordset[0].parentNode.nodeName);
        if (nn.match(/^(LinearRing)/i) || nn.match(/^(gml:LinearRing)/i)) {
            ...
            if (typeof name == "undefined") { name = that.unnamedarea; }
            if (!!that.opts.createpolygon) {
                that.opts.createpolygon(lines, color, width, opacity, fillcolor, fillOpacity, pbounds, name, desc, idx, visible, fill, outline, kml_id, zindex);
            } else {
                that.createPolygon(lines, color, width, opacity, fillcolor, fillOpacity, pbounds, name, desc, idx, visible, fill, outline, kml_id, zindex);
            }
        }
    };
GeoXml.prototype.makeIcon = function(currstyle, href, myscale, hotspot){
	...
	if (!!href) {
		  ...
		  var bicon = this.opts.baseicon;
		   ...
		   if(this.opts.baseicon.origin){
				tempstyle.origin = this.opts.baseicon.origin;
				}
			else {
				tempstyle.origin = new google.maps.Point(0,0);
				}
		   ...
		   if(this.opts.baseicon.scaledSize){
			...
			//alert(" size:"+tempstyle.size+" - anchor:"+tempstyle.anchor+" - scaled:"+tempstyle.scaledSize+" - origin:"+tempstyle.origin);
		   tempstyle.url = href;
		  } else {
...
GeoXml.prototype.handleStyle = function(style,sid,currstyle){
	 var that = this;
      ...
      if (icons.length > 0) {
		...
		if(!!!href && typeof currstyle != "undefined"){
			href = currstyle.url;
			}
		if (href){
...
GeoXml.prototype.processing = function(xmlDoc,title, latlon, desc, sbid) {
   ...
   if(basename == "kml") {	
	...
	if(!that.opts.sidebarid){
 	...
 	if (that.forcezoom && !that.basesidebar) {
        	that.map.fitBounds(that.bounds);
      		}
    	}
};
GeoXml.prototype.createFolder = function(idx, title, sbid, icon, desc, snippet, keepopen, visible){ 	  
	...
	var folderid = myvar+'_folder'+ idx;
	var obj = Lance$(folderid);
	if(obj && typeof obj != "undefined"){ }
		...
		desc = escape(desc);
		var htm = "";// = '<ul>';
		if (!!!this.suppressfolders) {
			//htm = '';
			htm += '<input type="checkbox" id="'+myvar+''+idx+'FCB" style="vertical-align:middle" ';
			htm += checked;
			//htm += 'onclick="'+myvar+'.toggleContents('+idx+',this.checked)">';
			htm += 'onclick="'+myvar+'.toggleAllNames(\''+folderid+'\',this.checked)">';
			htm += '&nbsp;<span title="'+snippet+'" id="'+myvar+'TB'+idx+'" oncontextmenu=\"'+myvar+'.saveJSON('+idx+');\" onclick="'+myvar+'.toggleFolder('+idx+')" style=\"'+fw+'\">';
			htm += '<img id=\"'+myvar+'FB'+idx+'\" style=\"vertical-align:text-top;padding:0;margin:0\" height=\"'+this.sidebariconheight+'\" border=\"0\" src="'+icon+'" /></span>&nbsp;';
			htm += '<a href="#" onclick="'+myvar+'.toggleFolder('+idx+')">';
			htm +=	title + '</a>';
			htm += '<ul><div id=\"'+folderid+'\" style="'+disp+'"></div></ul>'; //;
			}
		else {
			if (folderid == (this.basesidebar+"1")){
				htm = "<ul><span id='"+this.basesidebar+"_labels'></span>";
				}
		 	htm += '<div id=\"'+folderid+'\" style="'+disp+'"></div>'; 
			if (folderid == (this.basesidebar + "1")){
				htm += "</ul>";
				}
			}
		...
		/*
		var htm = '<ul><input type="checkbox" id="'+myvar+''+idx+'FCB" style="vertical-align:middle" ';
		htm += checked;
		//htm += 'onclick="'+myvar+'.toggleContents('+idx+',this.checked)">';
		htm += 'onclick="'+myvar+'.toggleAllNames(\''+folderid+'\',this.checked)">';
		htm += '&nbsp;<span title="'+snippet+'" id="'+myvar+'TB'+idx+'" oncontextmenu=\"'+myvar+'.saveJSON('+idx+');\" onclick="'+myvar+'.toggleFolder('+idx+')" style=\"'+fw+'\">';
		htm += '<img id=\"'+myvar+'FB'+idx+'\" style=\"vertical-align:text-top;padding:0;margin:0\" height=\"'+this.sidebariconheight+'\" border=\"0\" src="'+icon+'" /></span>&nbsp;';
		htm += '<a href="#" onclick="'+myvar+'.toggleFolder('+idx+')">' + title + '</a><br><div id=\"'+folderid+'\" style="'+disp+'"></div></ul>';
		*/
		
		if(sb){ sb.innerHTML = sb.innerHTML + htm; }
		if(this.tilesetObj){
			this.tilesetObj.folderCBNames.push(myvar+''+idx+'FCB');
			this.tilesetObj.folderIds.push(folderid);
			}
		else {
			this.folderCBNames.push(myvar+''+idx+'FCB');
...
GeoXml.prototype.toggleAllNames = function(idx,show){
	var z = this.tiledZoom;
	var folder = document.getElementById(idx);
	var myvar = this.myvar;
	if(this.tileset){
	...
	if(folder){
		for(var n=0;n<folder.names.length; n++){
			...
			this.showHideName(escape(folder.names[n]),show);
			}
		}
	else {return; }
	};	
...
OverlayManager.prototype.AddMarker = function (marker, title, idx, sidebar, visible, forcevisible) { 
    ...
    marker.hidden = false;
	marker.idx = escape(title+"_"+idx);
	var filtered = false;
	for(var n=0;n<this.paren.filteredNames.length;n++){
		if(marker.idx == this.paren.filteredNames[n]) {
			visible = false;
			filtered = true;
			}
		}
...
OverlayManager.Display = function (overlaymanager){
    ...
    for(k = 0; k< overlaymanager.folderhtml.length ; k++ ){	
		...
		if(con < curlen){
			var destid;
			if(overlaymanager.paren.tileset){
				destid = overlaymanager.paren.tileset+"_folder"+k;
				}	
			else {
				destid = overlaymanager.paren.myvar+"_folder"+k;
				}

			var dest = Lance$(destid);
			if(dest){
				if(typeof dest.names == "undefined"){
					dest.names = [];
					}
				if(overlaymanager.paren.opts.sortbyname){
					...
					for(l=0; l<curlen; l++){
						...
						if(bits[1]!=""){
							elemname = bits[1]+'_'+k;
							if (overlaymanager.paren.showUnique || overlaymanager.paren.tileset) {
								for (var n=0;n<dest.names.length;n++){
									if(elemname == dest.names[n]){
											exclude = true;
											}
										}
									}
							...
							bits[1] = "unamed element";
							exclude = false;
							}
						if(!exclude) {
							dest.names.push(elemname);
							content += overlaymanager.paren.sidebarfn(bits[0],bits[1],bits[2],bits[3],bits[4],bits[5],bits[6],bits[7],k); 
							}
						}
					}
				else {
				...
				if (content && content != ""){
					var core = dest.innerHTML;
					if(core && typeof core == "string"){
						var nc = document.createElement("span");
						nc.innerHTML = content;
						dest.appendChild(nc);
						} 
					else { dest.innerHTML = content; }
					}
					
				if(overlaymanager.paren.forcefoldersopen){
					dest.style.display = "";
				//	dest.parentNode.parentNode.style.display = "";
					overlaymanager.paren.kml[k].open = true;
					}
				if(overlaymanager.paren.forcefoldersclosed){
					dest.style.display = "none";
					dest.parentNode.parentNode.style.display = "none"; //keeps parent node forced closed.
					//console.log(destid+" forced closed");
					overlaymanager.paren.kml[k].open = false;
					}
  ...
  // Is this the last file to be processed?
	...
	if(update_side && count>0){
		 if (overlaymanager.paren.progress == 0) {
			...
			if (overlaymanager.paren.forcezoom) {
...
OverlayManager.PopUp = function ( group )
   ...
   // overlaymanager.map.closeInfoWindow(); close Last Marker
	...
	if(this.paren.tileset){
		this.paren.tileset.lastMarker = overlaymanager.marker;
		this.paren.tileset.lastMarker.infoWindow.open(this.paren.map);
		}
	else {
	...
	if (this.paren.singleInfoWindow) {
		if(this.paren.tilesetObj){ 
			if(!!this.paren.tilesetObj.lastMarker && !!this.paren.tilesetObj.lastMarker.infoWindow){
				this.paren.tilesetObj.lastMarker.infoWindow.close();
				}
			}
		else {
			if(!!this.paren.lastMarker && !!this.paren.lastMarker.infoWindow){
    ...
    try {
        ...
        if (cmlurl.substring(2, 3) == ":" && ! useLegacyLocalLoad) {
          ...
          GeoXml.Label = function(pos, txt, cls, map, style){
			...
			//	if (console){ console.log("h="+h); }
                div.style.left = parseInt(position.x - w/2) + 'px';
                div.style.top = parseInt(position.y - h/2) + 'px';
				}
